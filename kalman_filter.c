...
int8_t Matrix_Add(const mat *pSrcA, const mat *pSrcB, mat *pDst)
{
    if (pSrcA->numRows != pSrcB->numRows || pSrcA->numCols != pSrcB->numCols ||
        pSrcA->numRows != pDst->numRows || pSrcA->numCols != pDst->numCols) return -1;
...
{
    if (pSrcA->numCols != pSrcB->numRows || pSrcA->numRows != pDst->numRows || pSrcB->numCols != pDst->numCols) return -1;
...
{
    if (pSrc->numRows != pDst->numCols || pSrc->numCols != pDst->numRows) return -1;
...
{
    if (pSrc->numRows != pSrc->numCols || pDst->numRows != pDst->numCols || pSrc->numRows != pDst->numRows) return -1;
    uint16_t n = pSrc->numRows;
    float *aug = (float *)user_malloc(n * 2 * n * sizeof(float));
    if (!aug) return -1;

    for (uint16_t i = 0; i < n; i++) {
        for (uint16_t j = 0; j < n; j++) {
            aug[i * 2 * n + j] = pSrc->pData[i * n + j];
    ...
    for (uint16_t i = 0; i < n; i++) {
        ...
        if (fabs(pivot) < 1e-6f) {
            uint16_t swap_row = i + 1;
            while (swap_row < n && fabs(aug[swap_row * 2 * n + i]) < 1e-6f) swap_row++;
            if (swap_row == n) {
                free(aug);
                return -1;
            }
            for (uint16_t k = 0; k < 2 * n; k++) {
                float temp = aug[i * 2 * n + k];
                aug[i * 2 * n + k] = aug[swap_row * 2 * n + k];
                aug[swap_row * 2 * n + k] = temp;
            }
            pivot = aug[i * 2 * n + i];
        }

        for (uint16_t j = 0; j < 2 * n; j++) aug[i * 2 * n + j] /= pivot;
    ...
    for (uint16_t i = 0; i < n; i++) {
        for (uint16_t j = 0; j < n; j++) {
            pDst->pData[i * n + j] = aug[i * 2 * n + (j + n)];
        }
    }
